...
m01 <-lmer(value ~ q1*illusion + (1|subj), data=df)
res01 = anova(m01)
print(res01)

df2<-subset(df,df$illusion=="0")
m02 <-lmer(value ~ q1 + (1|subj), data=df2)
summary(m02)

df3<-subset(df,df$illusion=="1")
m03 <-lmer(value ~ q1 + (1|subj), data=df3)
summary(m03)

m04 <-lmer(value ~ illusion + (1|subj), data=df)
summary(m04)
...
eff = effect("q1*illusion",m01)

illusion_names <- as_labeller(c(`q1`="No Illusion",`illusion`="Illusion"))
plot_model(m01, type = "pred", terms = c("q1", "illusion"), title = "", axis.title =c("Scores Statement 1","Contrast"))+
  facet_wrap(~fct_rev(c("q1", "illusion")),nrow = 1, labeller = illusion_names)+
...
# for (roi in 1:21) {
  ...
  m04 <-lmer(value ~ q1 + (1|subj), data=df)
  summary(m04)
...
ggsave(paste("association_q1_illusion_modelB_roi",as.character(roi),".jpg"), path = path, width = 17.5, height = 17.5, units = "cm", device='jpg', dpi=700)
...
# Save figure Color
path='D:/VHI/Analysis/ROI_analysis/Associations/Figures';
ggsave(paste("association_q1_illusion_modelB_roi",as.character(roi),"_bw.jpg"), path = path, width = 17.5, height = 17.5, units = "cm", device='jpg', dpi=700)
...
for (roi in 1:21) {
  ...
  m01 <-lmer(value ~ q1 + (1|subj), data=df)
  res01 = anova(m01)
  print(res01)
