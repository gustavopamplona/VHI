...
for (roi in 1:21) {
  ...
  m01 <-lmer(value ~ q3*illusion + (1|subj), data=df)
  res01 = anova(m01)
  print(res01)
...
# for (roi in 1:21) {
  ...
  m02 <-lmer(value ~ q3 + (1|subj), data=df2)
  summary(m02)
  
  df3<-subset(df,df$illusion=="1")
  m03 <-lmer(value ~ q3 + (1|subj), data=df3)
  summary(m03)
  
  m04 <-lmer(value ~ illusion + (1|subj), data=df)
  summary(m04)
  
  m05 <-lmer(value ~ q3 + (1|subj), data=df)
  summary(m05)
...
# install.packages("effects")
# library(effects)
eff = effect("q3*illusion",m01)

illusion_names <- as_labeller(c(`q3`="No Illusion",`illusion`="Illusion"))
plot_model(m01, type = "pred", terms = c("q3", "illusion"), title = "", axis.title =c("Scores Statement 3","Contrast"))+
  facet_wrap(~fct_rev(c("q3", "illusion")),nrow = 1, labeller = illusion_names)+
  geom_line(size = .8)+
...
ggsave(paste("association_q3_illusion_modelB_roi",as.character(roi),".jpg"), path = path, width = 17.5, height = 17.5, units = "cm", device='jpg', dpi=700)
...
for (roi in 1:21) {
  ...
  m01 <-lmer(value ~ q3 + (1|subj), data=df)
  res01 = anova(m01)
  print(res01)
