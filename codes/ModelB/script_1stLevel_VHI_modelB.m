% Define and estimate 1st level betas for VHI - second half stimulation block

% Gustavo Pamplona, 25.01.23
...
for subj=first_subj:n_subj
    ...
    if ~nnz(strcmp(subject_folder(subj).name,remove_subjs))
        ...
        for run=1:n_runs
            ...
            directory=[data_folder filesep subject_folder(subj).name filesep 'Func\Func' num2str(run) filesep '1stLevel_modelB3'];
            func_source=[data_folder filesep subject_folder(subj).name filesep 'Func' filesep 'Func' num2str(run)];
            mult_cond=[data_folder filesep subject_folder(subj).name filesep 'Cond' filesep subject_folder(subj).name '_run' num2str(run) '_B2.mat'];
            ...
            % batch
            load('D:\VHI\Analysis\ModelB\batch_1stLevel_VHI_modelB.mat')
