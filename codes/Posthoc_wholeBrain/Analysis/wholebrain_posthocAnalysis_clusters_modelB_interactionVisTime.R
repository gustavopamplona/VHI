...
pathFolder = "D:/VHI/Analysis/ROI_analysis/Posthoc_wholeBrain/ModelB/05-Interaction time x vis/Clusters/Selected/tables/"
...
ggplot(df, aes(y=value, x=time), labeller = time_names) +
  geom_boxplot(aes(fill = interaction(vis,time)))+
  theme_bw()+
  scale_x_discrete(labels=c("firstHalf" = "Early", "secondHalf" = "Late"))+
  scale_fill_manual(values=c("#954595", "#C488C4", "#E4C9E4", "#954595", "#C488C4", "#E4C9E4"))+ #blue: 4545e5 8888ff c9c9ff #red: e54545 ff8888 ffc9c9 # purple: 954595 C488C4 E4C9E4
...
# Save figure Color
path='D:/VHI/Analysis/ROI_analysis/Posthoc_wholeBrain/ModelB/05-Interaction time x vis/Clusters/Selected/figures';
ggsave(paste("InteractionTimeVis_modelB_roi",as.character(roi),".jpg",sep=""), path = path, width = 17.5, height = 17.5, units = "cm", device='jpg', dpi=700)

ggplot(df, aes(y=value, x=time)) +
  theme_bw()+
  scale_fill_manual(values=c("#656565", "#A0A0A0", "#D4D4D4", "#656565", "#A0A0A0", "#D4D4D4"))+ #gray: 656565, A0A0A0, D4D4D4
  geom_boxplot_pattern(aes(fill = interaction(vis,time)),
                       pattern = c("none", "none", "none","none", "none", "none"),
                       pattern_angle = c(0,0,0,0,0,0),
                       colour          = 'black',
                       pattern_density = .1,
                       pattern_fill    = 'black',
...
# Save figure BW
path='D:/VHI/Analysis/ROI_analysis/Posthoc_wholeBrain/ModelB/05-Interaction time x vis/Clusters/Selected/figures';
ggsave(paste("InteractionTimeVis_modelB_roi",as.character(roi),"_bw.jpg",sep=""), path = path, width = 17.5, height = 17.5, units = "cm", device='jpg', dpi=700)
...
print(describeBy(df$value, df$vis:df$time), digits = 4)
